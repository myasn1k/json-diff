...
def main(argv):
    ...
    try:
        ...
        SlackNotification.send_slack2ctis_error_notification(
    ...
    for e in reversed(conversation_history):
        ...
        if not "Differences detected" in str(e):
            continue
        ...
        try:
            added = e["attachments"][0]["blocks"][1]["fields"][0]["text"].split("\n")[1:]
            ctis = CTIS(Config["slack_to_ctis"]["ctis"]["url"],
                    Config["slack_to_ctis"]["ctis"]["username"],
                    Config["slack_to_ctis"]["ctis"]["password"])
            ctis.upload_slack2ctis(added, Config["slack_to_ctis"]["ctis"]["actor_name"],
                    Config["slack_to_ctis"]["ctis"]["operation_name"],
                    Config["slack_to_ctis"]["ctis"]["operation_description"])
        except:
            logging.error(f"Failed uploading to ctis")
            SlackNotification.send_slack2ctis_error_notification(
                    Config["slack_to_ctis"]["slack_error_url"],
